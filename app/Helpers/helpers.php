...
use App\Models\RoleHasTemplate;
use Config;
...
class Helper
{
    ...
    {
        // get role uuid
        $role_uuid  = auth()->user()->roles;

        // generate menu template 
        $menu   = [];
        foreach ($role_uuid as $key => $role) {
            // disini membuat array list untuk menu sidebar
            $template   = RoleHasTemplate::with('template.templateDetail.page')->has('template')->where('role_id', $role->id)->orderBy('sequence', 'ASC')->get();
            // dd($template);
            if(!empty($template)){
                foreach ($template as $k => $v) {
                    $menu[$v->template->id]['url']        = 'dashboard/'. $v->template->id . '/' .  $v->template->name  .'/'. encrypt('1');
                    $menu[$v->template->id]['name']       = $v->template->name;
                    $menu[$v->template->id]['icon']       = $v->template->icon;
                    $menu[$v->template->id]['slug']       = 'dashboard-' . $v->template->name;
                    $menu[$v->template->id]['level']      = $v->template->level;
                    $menu[$v->template->id]['parent_id']  = $v->template->parent_id;
                }
            }
        }
        // membuat nested array sesuai parent nya 
        $menu = makeNested($menu);
